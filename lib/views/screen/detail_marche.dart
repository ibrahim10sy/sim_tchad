...
class DetailMarche extends StatefulWidget {
  ...
  DetailMarche({super.key, this.enqueteCollecte});
...
class _DetailMarcheState extends State<DetailMarche> {
  ...
  Enqueteur? enqueteur;
  List<PrixMarche> allFiches = []; // Liste originale
  List<PrixMarche> filteredFiches = []; // Liste pour l'affichage
  String searchQuery = "";
  bool isLoad = false;
  ...
  Future<void> _fetchDataLocal() async {
    ...
    try {
      print("numfiche ${widget.enqueteCollecte!.numFiche}");
      final MarchesData = await DatabaseService.getPrixMarcheByNum(
          widget.enqueteCollecte!.numFiche);
      setState(() {
        allFiches = MarchesData;
        print(allFiches.toList());
  ...
  Future<void> handleSync() async {
    ...
    try {
      await syncDataMarcheByFicheServer(
          widget.enqueteCollecte!.enqueteur!, widget.enqueteCollecte!.numFiche);
    ...
    final result = await Navigator.push(
        ...
        MaterialPageRoute(
            builder: (_) => AddProduitMarche(
                isEdit: true, enqueteCollecte: en, prixMarche: p)));
    log(result.toString());
    if (result == true) {
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          Column(
            children: [
              _buildInfoEnqueteHeader(), // En-tête avec infos EnqueteMarche
                              ...
                              itemBuilder: (context, index) {
                                return ProduitCollecteCard(
                                  ...
                                  onEdit: () => _getResultFromNextScreens(
                                      context,
                                      widget.enqueteCollecte!,
                                      filteredFiches[index]),
                                  onDelete: () => _showDeleteConfirm(
                                      filteredFiches[index].idPrixMarche!),
                                );
                              },
                            ),
                          ),
  ...
  Widget _buildInfoEnqueteHeader() {
    final e = widget.enqueteCollecte;
    return Container(
          ...
          children: [
            Row(
              children: [
                ...
                Text(e?.marche.nomMarche ?? "Marche inconnu",
